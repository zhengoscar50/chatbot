...
class FakeAgentService:
    ...
    def get_owned(self, agent_id, owner_id):
        return {"id": agent_id, "owner_id": owner_id} if self.owned else None


class FakeSessionService:
    def create_session(self, owner_id, agent_id, name=None):
        return {"id": "s1", "name": name or "New session", "agent_id": agent_id}
...
def build_app(agent_owned=True):
    app = FastAPI()
    app.include_router(sessions_route.router)
    app.dependency_overrides[get_session_service] = lambda: FakeSessionService()
    app.dependency_overrides[get_agent_service] = lambda: FakeAgentService(agent_owned)
...
def test_create_session_returns_id_and_name():
    r = TestClient(build_app()).post("/sessions", json={"agent_id": "ag-1", "name": "Taxes"})
    assert r.status_code == 200
    assert r.json() == {"id": "s1", "name": "Taxes"}


def test_create_session_defaults_name_when_omitted():
    r = TestClient(build_app()).post("/sessions", json={"agent_id": "ag-1"})
    assert r.status_code == 200
...
def test_create_session_requires_an_agent_id():
    assert TestClient(build_app()).post("/sessions", json={}).status_code == 422


def test_create_session_404_for_another_users_agent():
    r = TestClient(build_app(agent_owned=False)).post("/sessions", json={"agent_id": "ag-x"})
    assert r.status_code == 404
